...
func newTCPTransport(socket net.Conn, timeout time.Duration, notCheckTxnId bool, customLogger *log.Logger) (tt *tcpTransport) {
	tt = &tcpTransport{
		socket:        socket,
		timeout:       timeout,
		notCheckTxnId: notCheckTxnId,
...
func (tt *tcpTransport) readResponse() (res *pdu, err error) {
	...
	for {
		...
		if !tt.notCheckTxnId && tt.lastTxnId != txnId {
