...
func (mc *ModbusClient) Open() (err error) {
	...
	case modbusTCP:
		...
		mc.transport = newTCPTransport(sock, mc.conf.Timeout, mc.conf.NotCheckTxnId, mc.conf.Logger)
	...
	case modbusTCPOverTLS:
		...
		mc.transport = newTCPTransport(
			newTLSSockWrapper(sock), mc.conf.Timeout, mc.conf.NotCheckTxnId, mc.conf.Logger)
	...
	case modbusTCPOverUDP:
		...
		// packets byte per byte
		mc.transport = newTCPTransport(
			newUDPSockWrapper(sock), mc.conf.Timeout, mc.conf.NotCheckTxnId, mc.conf.Logger)
